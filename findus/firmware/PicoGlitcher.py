...
class PicoGlitcher():
    ...
    def __arm_common(self):
        ...
        if self.trigger_mode == "tio":
            ...
            if not self.trigger_inverting:
                sm1_func = Statemachines.tio_trigger_with_dead_time_rising_edge
            else:
                sm1_func = Statemachines.tio_trigger_with_dead_time_falling_edge
            ...
            if self.condition == "rising":
                sm2_func = Statemachines.block_rising_condition
            else:
                sm2_func = Statemachines.block_falling_condition
            ...
            # state machine that checks the trigger condition
            self.sm1.init(Statemachines.uart_trigger, freq=self.baudrate * 8, in_base=self.pin_trigger)
    ...
    def arm(self, delay:int, length:int, number_of_pulses:int = 1, delay_between:int = 0):
        ...
        if number_of_pulses == 1:
            ...
            # state machine that emits the glitch if the trigger condition is met
            self.sm0.init(Statemachines.glitch_burst, freq=self.frequency, set_base=self.pin_glitch, sideset_base=self.pin_glitch_en)
            # push delay and length (in nano seconds) into the fifo of the statemachine
            self.sm0.put(int(delay) // (1_000_000_000 // self.frequency))
    ...
    def arm_double(self, delay1:int, length1:int, delay2:int, length2:int):
        ...
        if pulse1_length > 2**10 or pulse2_length > 2**10:
            raise Exception("Pulse length exceeds maximum value.")
        if pulse1_delay > 2**22 or pulse2_delay > 2**22:
            raise Exception("Pulse delay exceeds maximum value.")
    ...
    def arm_multiplexing(self, delay:int, mul_config:dict, vinit:str = "config"):
        ...
        self.sm0.active(0)
        if vinit == "config":
            self.sm0.init(Statemachines.multiplex, freq=self.frequency, set_base=self.pin_glitch, out_base=self.pin_glitch, sideset_base=self.pin_glitch_en)
        elif vinit == "VI1":
            self.sm0.init(Statemachines.multiplex_vin1, freq=self.frequency, set_base=self.pin_glitch, out_base=self.pin_glitch, sideset_base=self.pin_glitch_en)
        elif vinit == "VI2":
            self.sm0.init(Statemachines.multiplex_vin2, freq=self.frequency, set_base=self.pin_glitch, out_base=self.pin_glitch, sideset_base=self.pin_glitch_en)
    ...
    def change_config_and_apply(self, key:str, value:int|float|str):
        ...
        with open("config.json", "r") as file:
            self.config = ujson.load(file)
        # reload the global variables and Statemachines
